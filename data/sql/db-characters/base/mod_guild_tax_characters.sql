...
-- own deposits, their withdrawals, what the bank paid for their repairs and the tax the module
-- deposited for them. One row per guild and character, created on the first booking; shown by
-- ".guild ledger".
...
CREATE TABLE IF NOT EXISTS `guild_member_ledger` (
  ...
  `repairs` BIGINT UNSIGNED NOT NULL DEFAULT 0,
